...
| Settings center | `kiosk-host/host/configcenter.py` | sidebar-nav Settings hub (credentials editor with TOTP generation, vault I/O off-GTK-thread, security lock enrolment) |
| Health probe | `kiosk-host/host/health.py` | launcher status dot / boot health: cheap sync checks + threaded slow probe for vault/VPN readiness |
| VPN log viewer | `kiosk-host/host/vpn_log_viewer.py` | live VPN log viewer streaming `journalctl -u forti-vpn.service -f` with Reconnect/Clear controls and per-name filter |
| Login memory | `kiosk-host/host/loginmemory.py` | domain-login memory store (domain → vault item name, `$SOC_STATE_DIR/domain_logins.json`) so re-pointed panels auto-login with the known item |
