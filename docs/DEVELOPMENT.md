...
- optional: `docker` to exercise the real Vaultwarden path (`litebw` is pure
  Python and bundled — the legacy `rbw` is optional)
...
dev/seed-ciphers.py          seeds Vaultwarden via REST (rbw `add` needs a TTY; litebw is read-only)
...
so the whole host runs without Vaultwarden. The **litebw backend** (default, prod)
is a pure-Python, `rbw`-compatible Vaultwarden client (no Rust toolchain — works
on the 1 GB Pi); the legacy **rbw backend** shells out to `rbw` and stays
selectable. `dev/run/` is gitignored — all runtime state lives there.
...
| `SOC_VAULT_BACKEND` | `litebw` (default, prod), `rbw` (legacy), or `dev` (JSON file) |
