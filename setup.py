...
def section_vpn(prev) -> dict:
    ...
    if vtype == "inode":
        ...
        # Advanced iNode flags
        note("Advanced: EAD sends a host-check result after login (required by some gateways); "
             "split tunnel routes only the gateway's subnets through the VPN (recommended).")
        ead = ask_bool("send EAD host-check ack after login?",
                       bool(pv.get("ead", False)))
        split = ask_bool("split tunnel — only route gateway subnets (recommended)?",
                         pv.get("split_tunnel", True))
        captcha = ask_int("CAPTCHA auto-retries (0 = default, higher helps with noisy fonts)",
                          int(pv.get("captcha_retries", 40)), lo=0)
        cafile = ask("custom CA bundle path (blank = system CAs)", pv.get("ca_file", ""))
        cert = ask("mutual-TLS client cert PEM (blank = none)", pv.get("client_cert", ""))
        ckey = ""
        if cert:
            ckey = ask("mutual-TLS client key PEM", pv.get("client_key", ""))
        mac = ask("override MAC sent to gateway (blank = auto)", pv.get("mac", ""))
        lang = ask("auth language [cn/en]", pv.get("language", "cn"))
        note("SPA/SDP (Zero-Trust): needed when the SSL VPN port is firewalled until a valid "
             "HOTP knock arrives. Supply the per-client key + AID from your enrolled client.")
        spa_key = ask("SPA HOTP key hex (blank = no knock)", pv.get("spa_key", ""))
        spa_aid = ""
        spa_ports = ""
        spa_kp = 0
        if spa_key:
            spa_aid = ask("SPA client AID hex", pv.get("spa_aid", ""))
            spa_ports = ask("SPA knock ports (comma-sep, default 443)", pv.get("spa_ports", ""))
            spa_kp = ask_int("SPA knock UDP port", int(pv.get("spa_knock_port", 8000)))
        probe = ask("ready_probe host:port the host waits on (blank = none)",
                    pv.get("ready_probe", ""),
                    validate=lambda s: None if not s.strip() else v_hostport(s))
        ...
        return dict(enabled=True, type="inode", gateway=gateway, port=port,
                    ...
                    trusted_cert=trusted, insecure=insecure,
                    ead=ead, split_tunnel=split, captcha_retries=captcha,
                    ca_file=cafile, client_cert=cert, client_key=ckey,
                    mac=mac, language=lang,
                    spa_key=spa_key, spa_aid=spa_aid, spa_ports=spa_ports,
                    spa_knock_port=spa_kp, ready_probe=probe,
