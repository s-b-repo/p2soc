...
def render_wall_unit(e: dict, *, user: str = "soc",
    ...
    L += [
        "",
        f"ExecStartPre=+/bin/sh -c '/usr/bin/install -d -m 0700 -o {user} -g {user} /run/user/$(id -u {user})'",
        ...
        "# multi-hundred-MB cores onto the SD card.",
        "LimitCORE=0",
        ...
        "# exhausting the 1 GB Pi.",
        "LimitNPROC=256",
        "LimitNOFILE=8192",
        "TasksMax=512",
        "# SEC-11: defense-in-depth (safe for the browser sandbox).",
        "ProtectClock=yes",
        "ProtectHostname=yes",
