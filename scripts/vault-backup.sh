...
# on the command line. Add --fips to backup with the PBKDF2 (FIPS-approved) KDF.
set -euo pipefail
...
case "$cmd" in
  backup)
    ...
    shift || true; [ "${1:-}" = "backup" ] && shift || true
  ...
  restore)
    ...
    ;;
  *)
    echo "usage: $0 {backup [out.bak] [--fips] | restore <in.bak> [dest]}" >&2
    exit 2
    ;;
esac
