...
{ "SOC Forti":  {"username":"fortiuser","password":"placeholder"},
  "SOC OVPN":   {"username":"ovpnuser","password":"placeholder"},
  "SOC WG":     {"username":"","password":"x",
                 "notes":"[Interface]\nPrivateKey = PLACEHOLDER_WG_KEY==\nAddress = 10.9.0.2/32\n"},
  "SOC iNode":  {"username":"inodeuser","password":"placeholder"} }
JSON
...
check("openvpn: creds injected over mgmt socket (not argv)", got == "ovpnuser:placeholder")
...
check("wireguard: key came from the vault Notes", "PLACEHOLDER_WG_KEY==" in seen)
...
check("inode: password via $H3C_SVPN_PASSWORD env (not argv)",
      pw == "placeholder" and "placeholder" not in argv)
