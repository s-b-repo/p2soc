...
dev-vault:  ## write dev/run/dev-vault.json (dev vault backend, no server needed)
	@mkdir -p dev/run
	@printf '%s\n' '{' \
	  '  "SOC Dev Panel 1": {"username": "viewer1", "password": "placeholder"},' \
	  '  "SOC Dev Panel 2": {"username": "viewer2", "password": "placeholder"},' \
	  '  "SOC Dev Panel 3": {"username": "viewer3", "password": "placeholder"},' \
	  '  "SOC Dev Panel 4": {"username": "viewer4", "password": "placeholder"},' \
	  '  "SOC Dev VPN": {"username": "vpnuser", "password": "placeholder"}' \
...
vault: .venv  ## start Vaultwarden in Docker + seed it via rbw (full vault path)
	...
	docker run -d --name soc-vaultwarden -e ADMIN_TOKEN=placeholder \
