...
def test_roundtrip_restore(tmp_path):
    ...
    backup.write_backup(src, out, "correct horse battery", fips=False)
...
def test_fips_mode_roundtrips_and_records_algo(tmp_path):
    src = _make_vault(tmp_path)
    blob = backup.make_backup(src, "fips-passphrase!", fips=True)
    assert blob[len(backup._MAGIC)] == ss._ALGO_PBKDF2
    # self-describing: opens with no FIPS env, algo taken from the blob
    inner = backup.open_backup(blob, "fips-passphrase!")
    with tarfile.open(fileobj=io.BytesIO(inner), mode="r:gz") as t:
        assert any(n.endswith("db.sqlite3") for n in t.getnames())
