...
def test_inode_helpers():
    ...
    assert config.inode_extra_args(vpn) == ["--", "--pin-sha256", "AA:BB:CC", "--split-tunnel"]
    assert config.inode_extra_args({"insecure": True}) == ["--", "--insecure", "--split-tunnel"]
    assert config.inode_extra_args({}) == ["--", "--split-tunnel"]
    assert config.inode_extra_args({"split_tunnel": False}) == []
